...
## v6 Update Note (2026-02-19)

Added `WHITE_LABEL_PLAN.md` — Phase 3.75 white-label branding before guest-facing pages. Key changes:
- **Backend**: 15 new fields on Hotel model — brand colors (primary/secondary/accent), fonts
  (heading/body), favicon, social links (Instagram/Facebook/Twitter/WhatsApp), OG image,
  footer text, legal URLs (terms/privacy). Hex color validation. Updated serializers.
- **Frontend types**: `Hotel` and `HotelSettings` interfaces extended with branding fields.
- **New components**: `FontCombobox` (Google Fonts picker), `BrandFontLoader` (dynamic
  `<link>` injection), `BrandPreview` (live preview in settings), `inject-brand-theme`
  utility (CSS variable injection on `<html>`).
- **Settings page**: New Branding / Social Links / Footer & Legal cards.
- **Guest layout**: `generateMetadata` for per-hotel title, favicon, OG image. Brand theme
  injected on mount. Guest footer component with social icons and legal links.
- **Phase 4**: All guest pages consume `--brand-primary`, `--brand-secondary`, `--brand-accent`
  CSS variables and `--brand-heading-font`, `--brand-body-font` for typography.
- **Phase 5**: PWA manifest `theme_color`/`background_color` dynamic per hotel.
- **Phase 6**: CSP headers allow Google Fonts CDN; OG tag validation added.

## v5 Update Note (2026-02-18)

Synced with `PRE_PHASE4_REVAMP.md` — the pre-Phase 4 admin UX overhaul plan. Key changes:
- **Types**: Added `ContentStatus` enum, `published_at`, `setup` flags in `DashboardStats`.
  `is_active` on Department/Experience is being replaced with `status` via a 3-step
  backward-compatible migration (see `PRE_PHASE4_REVAMP.md` for rollout steps).
- **Components**: Dialog-based `DepartmentForm`/`ExperienceForm` replaced with full-page
  editor routes. New hooks: `use-unsaved-changes`, `use-local-draft`. New: `SetupChecklist`.
- **Routes**: Experience editors nest under department slug:
  `/dashboard/departments/[deptSlug]/experiences/[id]/edit`.
- **Phase 3→4/5/6 integration tasks**: Updated to reference full-page editors, status
  workflow, mobile reorder buttons, and provisional preview routes.
- **Verification**: Updated to reflect full-page editors, status workflow, and new routes.
...
**Defense-in-depth (CSP + sanitized rendering):**
...
- **Descriptions stored as sanitized HTML** (Phase 3 enhancement): backend `bleach` strips all tags except an allowlist (`p`, `br`, `strong`, `em`, `s`, `h2`, `h3`, `ul`, `ol`, `li`, `blockquote`, `hr`) with zero attributes on every save. Frontend renders via Tiptap read-only `EditorContent` (ProseMirror contenteditable — no `dangerouslySetInnerHTML`). Admin dashboard card previews strip HTML to plain text. See `PHASE3_ENHANCEMENTS.md` §Security for full policy.
...
- `status` (CharField, choices: `DRAFT`/`PUBLISHED`/`UNPUBLISHED`, default `DRAFT`) — **v5: replaces `is_active`**. During the dual-write transition phase, both `status` and `is_active` exist on the model; `is_active` is computed from status (`PUBLISHED` → True, else False) and exposed as read-only in serializers. After transition completes, `is_active` is removed. See `PRE_PHASE4_REVAMP.md` for rollout steps.
- `published_at` (DateTimeField, nullable) — set to `timezone.now()` on first publish, updated on re-publish
...
- `status` (CharField, choices: `DRAFT`/`PUBLISHED`/`UNPUBLISHED`, default `DRAFT`) — **v5: same transition as Department**
...
| Method | URL | View | Purpose |
|--------|-----|------|---------|
| CRUD | `/hotels/{hotel_slug}/admin/departments/` | DepartmentViewSet | Manage departments. Slug auto-generated on create (read-only). Description sanitized via `bleach` (allowlisted HTML tags only). |
| PATCH | `/hotels/{hotel_slug}/admin/departments/reorder/` | DepartmentBulkReorder | Bulk update `display_order`. Body: `{ "order": [id, id, ...] }`. Server assigns `display_order` by array index. Atomic, hotel-scoped, 400 on duplicates/invalid IDs/empty list. |
| CRUD | `/hotels/{hotel_slug}/admin/departments/{dept_slug}/experiences/` | ExperienceViewSet | Manage experiences. Slug auto-generated on create (read-only). Description sanitized via `bleach`. |
| PATCH | `/hotels/{hotel_slug}/admin/departments/{dept_slug}/experiences/reorder/` | ExperienceBulkReorder | Bulk update `display_order`. Same contract as department reorder, scoped to department. |
| POST | `/hotels/{hotel_slug}/admin/departments/{dept_slug}/experiences/{exp_id}/images/` | ExperienceImageUpload | Upload gallery image. Auto-assigns next `display_order`. |
| PATCH | `/hotels/{hotel_slug}/admin/departments/{dept_slug}/experiences/{exp_id}/images/reorder/` | ExperienceImageReorder | Bulk reorder gallery images. Same atomic constraints. |
| DELETE | `/hotels/{hotel_slug}/admin/departments/{dept_slug}/experience-images/{pk}/` | ExperienceImageDelete | Delete gallery image. Hotel+dept scoped via queryset. |
...
- `DepartmentSerializer` — full CRUD fields, nested experiences for read, schedule (JSON validated). **slug is read-only** (auto-generated from name on create). **description sanitized** via `bleach` (allowlisted HTML tags: p, br, strong, em, s, h2, h3, ul, ol, li, blockquote, hr — no attributes).
- `ExperienceSerializer` — full CRUD fields, **validates `department.hotel == context['hotel']`**. **slug is read-only** (auto-generated). **description sanitized** same as Department. Includes `gallery_images` (read-only nested `ExperienceImageSerializer`).
- `ExperienceImageSerializer` — id, image, alt_text, display_order, created_at (id and created_at read-only)
...
bleach>=6.0  # HTML sanitization for department/experience descriptions (Phase 3 enhancement)
```
...
├── inject-brand-theme.ts # v6: CSS variable injection — sets --brand-primary, --brand-secondary, --brand-accent, --brand-heading-font, --brand-body-font on <html>
...
- `HotelHeader.tsx` — hotel logo, name, description — uses `--brand-primary` for background, `--brand-heading-font` for name
- `DepartmentCard.tsx` — photo, name, description, experience count, schedule badge ("Open today: 9-9")
- `ExperienceCard.tsx` — photo, name, price, timing, "Book" button — CTA uses `--brand-accent`
- `GuestFooter.tsx` — **v6: new**. Hotel `footer_text`, social icons (Instagram/Facebook/Twitter/WhatsApp linking to configured URLs), `terms_url`/`privacy_url` links. Hidden when no footer content configured.
- `BrandFontLoader.tsx` — **v6: new**. Injects Google Fonts `<link>` tags for `heading_font`/`body_font` + sets `--brand-heading-font`/`--brand-body-font` CSS variables. Fallbacks: serif (heading), sans-serif (body).
...
- ~~`DepartmentForm.tsx`~~ → **v5: replaced by full-page editor routes** (`/dashboard/departments/new`, `/dashboard/departments/[slug]/edit`). Mobile-first layout with sectioned cards. See `PRE_PHASE4_REVAMP.md` for editor wireframes.
- ~~`ExperienceForm.tsx`~~ → **v5: replaced by full-page editor routes** (`/dashboard/departments/[deptSlug]/experiences/new`, `/dashboard/departments/[deptSlug]/experiences/[id]/edit`). Routes nest under department slug so the URL is self-contained on hard refresh.
- `MemberTable.tsx` — list members (shows email, phone, role), invite new (email + optional phone), change role. **v5: adds mobile card layout below `md` breakpoint** — same data/actions, card-based instead of table.
...
- `SetupChecklist.tsx` — **v5: new**. Onboarding checklist shown on dashboard home when setup is incomplete. Driven by backend `setup` flags in `DashboardStats`. Dismissible per-hotel via localStorage.
- `ConfirmDialog.tsx` — **v5: new**. Themed confirmation modal (shadcn AlertDialog) replacing all `window.confirm()` calls. Supports destructive variant.
- `FontCombobox.tsx` — **v6: new**. Searchable combobox (Popover + Command pattern) with ~30 curated Google Fonts for hospitality. Separate heading/body lists. Shows font name in its own typeface as preview.
- `BrandPreview.tsx` — **v6: new**. Live preview card in settings page showing brand appearance: logo on primary_color bg, heading in heading_font, body text in body_font, CTA in accent_color. Updates in real-time.
...
**v5: New hooks (`src/hooks/`):**
- `use-unsaved-changes.ts` — guards against accidental navigation away from dirty editors. Uses `beforeunload` (tab close), `popstate` (browser back), and link interception via context (sidebar nav). App Router compatible — no Pages Router APIs.
- `use-local-draft.ts` — auto-saves form state to localStorage on debounced changes (1.5s). Restores on revisit with prompt. Clears on successful save.

### 2.7 PWA Setup

**Files to create:**
- `public/manifest.json` — app name, icons, display: standalone, theme color. **v6**: `theme_color` and `background_color` should be dynamic per hotel on guest pages (sourced from brand colors).
...
```
# Rich text editor (Phase 3 enhancements)
@tiptap/react @tiptap/starter-kit @tiptap/pm

# Drag-and-drop reordering (Phase 3 enhancements)
@dnd-kit/core @dnd-kit/sortable @dnd-kit/utilities

# shadcn/ui components added beyond base install (Phase 2+3):
# alert-dialog, command, popover, select, switch, table, textarea
# (installed via `npx shadcn@latest add <component>`)

# No new packages for white-label (Phase 3.75) — uses native <input type="color">,
# Intl.supportedValuesOf, existing shadcn Command/Popover, Google Fonts CDN <link> tags.
```
...
// v5: Content publishing status (replaces is_active on Department/Experience)
// Rolled out via 3-step backward-compatible migration — see PRE_PHASE4_REVAMP.md.
// During transition, admin serializers expose both is_active (read-only, computed)
// and status (writable). Frontend should prefer status when available.
...
export interface Department {
  ...
  is_ops: boolean;
  // v5: is_active replaced by status + published_at (see ContentStatus).
  // During migration transition, is_active may still be present (read-only, computed).
  status: ContentStatus; published_at: string | null;
  is_active?: boolean; // deprecated — transition only
}
...
export interface Experience {
  ...
  highlights: string[]; gallery_images: ExperienceImage[];
  // v5: is_active replaced by status + published_at (see ContentStatus).
  status: ContentStatus; published_at: string | null;
  is_active?: boolean; // deprecated — transition only
}
...
export interface ExperienceImage {
  id: number; image: string; alt_text: string;
...
export interface DashboardStats {
  ...
  // v5: Backend-computed setup completion flags for onboarding checklist.
  // All conditions are server-side booleans — frontend never guesses.
  setup: {
    settings_configured: boolean; // explicit flag on Hotel model
...
1. Install new Python packages (django-filter, celery, redis, django-celery-beat, django-celery-results, django-ratelimit, qrcode, pywebpush, py-vapid, requests, bleach)
...
#### Phase 3 Enhancements (see `PHASE3_ENHANCEMENTS.md` for full spec):
27a. Backend: auto-generate slugs from name on create (Department + Experience), make slug read-only in serializers
27b. Backend: `pip install bleach`, sanitize description HTML in DepartmentSerializer + ExperienceSerializer (`validate_description`)
27c. Backend: bulk reorder endpoints for departments + experiences (atomic, hotel-scoped, deterministic)
27d. Backend: ExperienceImage model + migration + upload/delete/reorder API (all admin + hotel + dept scoped)
27e. Frontend: install `@tiptap/react`, `@tiptap/starter-kit`, `@tiptap/pm`, `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities`; add shadcn `command` + `popover`
27f. Frontend: create shared components — RichTextEditor, SortableList, ImageUploadArea, TimezoneCombobox
27g. Frontend: rewrite departments page — remove slug, rich text description, drag-to-reorder, styled photo upload, timezone combobox, horizontal day overrides
27h. Frontend: rewrite experiences page — remove slug, rich text description, drag-to-reorder, styled uploads, gallery image manager, highlights helper text

#### Pre-Phase 4 Revamp (see `PRE_PHASE4_REVAMP.md` for full spec):
27i. Backend: Add `ContentStatus` enum (`DRAFT`/`PUBLISHED`/`UNPUBLISHED`) + `status` field + `published_at` to Department and Experience. 3-step backward-compatible migration: (1) dual-write with `is_active` compat, (2) frontend switches to `status`, (3) remove `is_active`.
27j. Backend: Add `settings_configured` boolean to Hotel model. Extend dashboard stats with `setup{}` flags for onboarding checklist.
27k. Frontend: Full-page editor routes replacing dialog-based forms. Department: `/dashboard/departments/new` and `/dashboard/departments/[slug]/edit`. Experience: `/dashboard/departments/[deptSlug]/experiences/new` and `/dashboard/departments/[deptSlug]/experiences/[id]/edit`.
27l. Frontend: `use-unsaved-changes` hook (beforeunload + popstate + link interception, App Router compatible) + `use-local-draft` hook (localStorage auto-save with debounce, restore prompt, clear on save).
27m. Frontend: Status workflow UI — status dropdown in editors, publish/unpublish actions, status badges on list cards, optional status filter tabs.
27n. Frontend: Mobile reorder fallback — up/down `ChevronUp`/`ChevronDown` buttons alongside drag handles in `SortableList`. Always visible, disabled at list boundaries.
27o. Frontend: Team page mobile card layout — card-based below `md` breakpoint, table above.
27p. Frontend: Setup checklist on dashboard home — driven by backend `setup{}` flags, dismissible per-hotel via localStorage.

### Phase 3.75: White-Label Branding (see `WHITE_LABEL_PLAN.md`)
27q. Backend: add 15 brand fields to Hotel model — `primary_color`, `secondary_color`, `accent_color` (hex, validated), `heading_font`, `body_font` (Google Font names), `favicon`, `og_image` (ImageField), `instagram_url`, `facebook_url`, `twitter_url`, `whatsapp_number`, `footer_text`, `terms_url`, `privacy_url`. Run migration.
27r. Backend: update `HotelPublicSerializer` (add all brand fields) + `HotelSettingsSerializer` (all editable).
27s. Backend: hex color validation (`RegexValidator` for `^#[0-9a-fA-F]{6}$`) on color fields.
27t. Frontend: extend `Hotel` + `HotelSettings` types in `concierge-types.ts` with all brand fields.
27u. Frontend: create `FontCombobox` component — searchable Google Fonts picker (Popover + Command pattern, ~30 curated hospitality fonts).
27v. Frontend: add Branding / Social Links / Footer & Legal cards to hotel settings page — color pickers, font comboboxes, ImageUploadArea for favicon/OG, text inputs for URLs.
27w. Frontend: create `inject-brand-theme.ts` utility — sets `--brand-primary`, `--brand-secondary`, `--brand-accent`, `--brand-heading-font`, `--brand-body-font` CSS variables on `<html>`.
27x. Frontend: create `BrandFontLoader` component — dynamically injects Google Fonts `<link>` tags based on hotel config. Uses `display=swap` to prevent FOIT.
27y. Frontend: create `BrandPreview` component — live preview card in settings showing brand colors, fonts, logo. Updates in real-time.

### Phase 4: Frontend — Guest-Facing (white-label aware)
32. Guest layout (`/h/[hotelSlug]/layout.tsx`) — `BrandFontLoader` + `injectBrandTheme()` on mount, `generateMetadata` for per-hotel title/favicon/OG image/theme-color.
33. Hotel landing page (departments grid with schedule badges) — brand colors/fonts via CSS variables. Reads `?qr=` param from URL (set by QR code scan); stores in page state and passes through to OTP verify flow.
...
33a. Render department descriptions as sanitized HTML on guest pages — use Tiptap read-only `EditorContent` (ProseMirror, no `dangerouslySetInnerHTML`) or a safe HTML renderer that only allows the bleach allowlist tags. Verify CSP blocks any residual inline script.
33b. Render experience descriptions as sanitized HTML on guest detail pages — same approach as 33a.
33c. Display `gallery_images` on experience detail page — image grid/carousel sorted by `display_order`. Lazy-load images. Preserve drag-reordered order from admin.
33d. Guest pages respect department/experience `display_order` from admin drag-and-drop ordering.
33e. **v5**: Guest pages only show `status='PUBLISHED'` departments and experiences (public API already filters). DRAFT and UNPUBLISHED content is invisible to guests.
33f. **v5 (provisional)**: Preview-as-guest — "Preview" button in full-page editors opens guest-facing view in new tab. For DRAFT content, append `?preview=true` with a short-lived preview token. Backend allows preview-token-authenticated requests to see draft content. Show banner: "Preview Mode — This content is not published yet." **Exact guest route paths are provisional** — will be finalized when Phase 4 routes stabilize. Editor reserves header space for the button, but it stays hidden/disabled until guest routes exist.
33g. **v6**: All guest pages consume brand CSS variables (`--brand-primary`, `--brand-secondary`, `--brand-accent`) — no hardcoded colors. Typography uses `--brand-heading-font` and `--brand-body-font`. Hotels without custom fonts get default serif/sans-serif fallbacks.
33h. **v6**: Guest page `<head>` metadata is dynamic per-hotel — title, description, favicon, OG image, theme-color all sourced from Hotel branding fields. Uses Next.js `generateMetadata`.

### Phase 5: Notifications + PWA
40. PWA manifest — **v6: `theme_color`/`background_color` dynamic per hotel** from brand colors. Manifest endpoint or build-time generation.
41. Service worker (`public/sw.js`) — push only, no PII in payloads, no offline cache
42. Push subscription flow (subscribe on dashboard login, unregister before logout — see S5)
43. Notification bell component + dropdown
44. Backend notifications on new request/escalation (push + optional escalation-only email/SMS fallback)

#### Phase 3→5 integration tasks (cache invalidation + setup flags):
41a. After admin reorders departments/experiences (bulk reorder PATCH), guest pages must reflect new order. If caching is used (CDN, service worker, or ISR), invalidate or revalidate the affected hotel landing and department pages.
41b. After gallery image upload/delete/reorder, invalidate cached experience detail pages that include `gallery_images`.
41c. Service worker must NOT cache admin API responses (reorder, image upload). Only cache static assets and optionally guest page HTML with short TTL.
41d. **v5**: After admin changes department/experience `status` (publish/unpublish), invalidate cached guest pages. Published content appears; unpublished content disappears.
41e. **v5**: Dashboard stats `setup{}` flags must update correctly after each admin action (create dept, invite member, publish, etc.) — verify checklist reflects changes on next dashboard visit.

### Phase 6: Polish
45. Request detail page with activity timeline
46. Request status update flow (only valid state transitions shown as buttons)
47. Escalation health indicator + degraded banner in dashboard (SUPERADMIN)
48. Mobile responsive dashboard
49. Error handling, loading states, empty states

#### Phase 3→6 integration tasks (accessibility + hardening + mobile + branding):
46a. DnD accessibility: ensure keyboard-only drag-and-drop works for all sortable lists (Tab to focus handle, Space/Enter to grab, Arrow keys to move, Space/Enter to drop). SortableList must include `KeyboardSensor` with `sortableKeyboardCoordinates`.
46b. Upload failure/retry UX: gallery image upload failures show inline error with retry button. Network errors during reorder show toast with option to retry. Optimistic reorder reverts on API failure.
46c. Rich-text sanitization regression test: create department/experience with known XSS payloads (`<script>`, `<img onerror>`, `<svg onload>`, `javascript:` URIs) → verify backend strips all, guest page renders safely. Add to verification checklist §11.
46d. Empty gallery state: show "No gallery images yet — upload to add" placeholder in experience full-page editor when `gallery_images` is empty.
46e. **v5**: Mobile reorder buttons (ChevronUp/ChevronDown) work correctly alongside drag handles — up disabled on first item, down disabled on last. Buttons call `onReorder` with swapped array.
46f. **v5**: Content completeness indicators on list page cards — subtle amber text showing missing fields (photo, description, etc.) for DRAFT and PUBLISHED items. Pure frontend logic, no backend changes.
46g. **v6**: CSP headers for guest route group must allow Google Fonts CDN (`fonts.googleapis.com`, `fonts.gstatic.com`) in `style-src` and `font-src` directives.
46h. **v6**: OG tag / social sharing validation — test each hotel's OG tags render correctly with Facebook Debugger / Twitter Card Validator. Verify favicon appears in browser tab.
46i. **v6**: Font loading performance — ensure `display=swap` prevents FOIT, fonts load in parallel with page content, no layout shift on font swap.
...
11. **CSP + sanitization test:**
    - Admin creates department/experience with description containing `<script>alert(1)</script>`, `<img onerror="alert(1)">`, `<svg onload="alert(1)">` → backend `bleach` strips all script/event handler tags, stores only allowlisted HTML
    - Guest page renders the sanitized description via Tiptap read-only or safe renderer (no `dangerouslySetInnerHTML`) → no script execution
    - Check response headers include strict CSP for guest routes (`script-src 'self'`)
    - v5: Admin dashboard card previews render formatted HTML via `HtmlContent` component (server-sanitized, `dangerouslySetInnerHTML` with prose styles) — not raw HTML tags as text
...
14. **Frontend test:**
    - Login → dashboard with stats + setup checklist (if incomplete)
    - v5: Navigate to `/dashboard/departments/new` → full-page editor loads, mobile-friendly layout
    - v5: Fill department form, press browser back → unsaved changes warning shown
    - v5: Close browser mid-edit, reopen same URL → local draft restored with prompt
    - v5: Create department as DRAFT → not visible on guest page; publish → appears on guest page
    - v5: Navigate to `/dashboard/departments/{deptSlug}/experiences/{id}/edit` → experience editor loads with correct data (hard refresh works — URL is self-contained)
    - v5: Reorder departments on mobile (375px) using up/down buttons → order persists after refresh
    - v5: View team page on mobile → card layout with all actions functional
    - Create department + experiences + publish → appears on guest page
...
21. **Gallery + rich-text guest rendering test (Phase 4):**
    - Guest department page renders sanitized HTML description (bold, lists, headings visible) — no raw HTML tags shown as text
    - Guest experience detail page renders sanitized HTML description identically
    - Guest experience detail page shows gallery images in correct `display_order` (matching admin drag order)
    - Guest department list respects admin-set `display_order` (drag-reordered departments appear in correct sequence)
    - Guest experience list within a department respects `display_order`
    - Inject `<script>`, `<img onerror>` in description via direct API call → guest page renders safely (tags stripped by backend, CSP blocks residual)
    - v5: Guest pages only show PUBLISHED departments/experiences — DRAFT and UNPUBLISHED are invisible
    - v5 (provisional): Preview button in editor opens guest view for DRAFT content with preview token + banner
22. **Cache invalidation test (Phase 5):**
    - Admin reorders departments → guest page reflects new order within acceptable staleness window (no stale CDN/ISR cache)
    - Admin uploads/deletes gallery image → guest experience detail shows updated gallery
    - Service worker does not cache admin API mutation responses (reorder, image upload/delete)
    - v5: Admin publishes/unpublishes department → guest page shows/hides accordingly after cache invalidation
    - v5: Dashboard stats `setup{}` flags update correctly after each admin action
23. **DnD accessibility + upload resilience test (Phase 6):**
    - Keyboard-only reorder: Tab to GripVertical handle → Space to grab → Arrow Up/Down to move → Space to drop → order persists
    - Screen reader announces drag state changes (grabbed, moved to position N, dropped)
    - Gallery image upload with simulated network failure → inline error shown with retry button
    - Reorder API failure (e.g., 500) → optimistic UI reverts to previous order, toast shown with retry option
    - Empty gallery state shows placeholder message in experience full-page editor
    - v5: Mobile up/down buttons reorder correctly — first item has up disabled, last has down disabled
    - v5: Content completeness indicators show missing fields on list cards for DRAFT and PUBLISHED items
24. **v5: Status workflow test:**
    - Create department (default DRAFT) → public API returns 404 for this department
    - Change status to PUBLISHED → `published_at` set to current time, public API returns department
    - Change status to UNPUBLISHED → public API returns 404, `published_at` preserved
    - Re-publish → `published_at` updated to new time
    - v5 transition: during dual-write phase, reading `is_active` returns `true` for PUBLISHED, `false` for DRAFT/UNPUBLISHED
    - v5 transition: old frontend sending `is_active=true` maps to PUBLISHED on backend
25. **v5: Unsaved changes + local draft test:**
    - Open department editor, make changes, press browser back → popstate intercepted, ConfirmDialog shown
    - Dismiss dialog → stays on editor. Confirm → navigates away, draft preserved in localStorage
    - Reopen same editor URL → "Restore draft?" prompt shown with timestamp
    - Accept restore → form populated with draft data. Discard → form shows server data
    - Save successfully → localStorage draft cleared
    - Close tab mid-edit → `beforeunload` fires browser-native prompt
    - Sidebar link clicked while dirty → interception context prevents navigation, shows dialog
